...
void G1BarrierSetC2::post_barrier(GraphKit* kit,
  ...
  if (val != nullptr) {
    ...
    Node *xor_res;
    if (StoreCachedCode)  {
      Node* grain_size_adr = __ makecon(TypeRawPtr::make((address)&G1HeapRegion::LogOfHRGrainBytes));
      // load this as a boolean so we read it as an unsigned byte
      Node* grain_size  = __ load(__ ctrl(), grain_size_adr, TypeInt::INT, T_BOOLEAN, Compile::AliasIdxRaw);
      xor_res = __ XorX( cast,  __ CastPX(__ ctrl(), val));
      xor_res = __ URShiftX( xor_res, grain_size);
    } else {
      xor_res = __ XorX( cast,  __ CastPX(__ ctrl(), val));
